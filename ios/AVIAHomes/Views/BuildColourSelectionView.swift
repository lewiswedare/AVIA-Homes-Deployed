...
struct BuildColourSelectionView: View {
    ...
    /// so clients get the same Approve / Decline experience for colour upgrade
    /// quotes as they do for spec item upgrade quotes.
    @ViewBuilder
    private var colourUpgradeQuotesCard: some View {
        ...
        if !pending.isEmpty {
            let total = pending.compactMap(\.cost).reduce(0, +)
                ...
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Colour Upgrade Quotes Ready")
                            .font(.neueCaptionMedium)
                            .foregroundStyle(AVIATheme.textPrimary)
                        Text("\(pending.count) upgrade\(pending.count == 1 ? "" : "s") awaiting your response")
                            .font(.neueCaption2)
                            .foregroundStyle(AVIATheme.textSecondary)
                    }
                    Spacer()
                    Text(AVIATheme.formatCost(total))
                        .font(.neueCorpMedium(18))
                        .foregroundStyle(AVIATheme.accent)
                }

                ForEach(pending, id: \.id) { sel in
                    quotedColourUpgradeRow(sel)
                }
            }
            .padding(14)
            .background(AVIATheme.cardBackground)
    ...
    private func quotedColourUpgradeRow(_ sel: BuildColourSelection) -> some View {
        let specItem = approvedItemsNeedingColour.first { $0.id == sel.buildSpecSelectionId }
        let name = specItem?.snapshotName ?? "Colour upgrade"
        ...
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 10) {
                ...
                Spacer()
                Text(AVIATheme.formatCost(sel.cost ?? 0))
