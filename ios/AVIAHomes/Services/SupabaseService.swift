...
class SupabaseService {
    ...
    func fetchDesignFavourites(userId: String) async -> [DesignFavouriteRow] {
        guard isConfigured else { return [] }
        do {
            let rows: [DesignFavouriteRow] = try await client
                .from("design_favourites")
                .select()
            ...
            print("[SupabaseService] fetchDesignFavourites FAILED for user=\(userId) — \(error)")
            return []
        }
    }

    func fetchAllDesignFavourites() async -> [DesignFavouriteRow] {
        guard isConfigured else { return [] }
        do {
            let rows: [DesignFavouriteRow] = try await client
                .from("design_favourites")
            ...
            print("[SupabaseService] fetchAllDesignFavourites FAILED — \(error)")
            return []
        }
    }

    @discardableResult
    func insertDesignFavourite(userId: String, designId: String) async -> Bool {
        guard isConfigured else { return false }
        let row = DesignFavouriteInsert(user_id: userId, design_id: designId)
        do {
            try await client
                .from("design_favourites")
                .insert(row)
            ...
            print("[SupabaseService] insertDesignFavourite FAILED — \(error)")
            return false
        }
    }

    @discardableResult
    func deleteDesignFavourite(userId: String, designId: String) async -> Bool {
        guard isConfigured else { return false }
        do {
            try await client
                .from("design_favourites")
            ...
            print("[SupabaseService] deleteDesignFavourite FAILED — \(error)")
            return false
        }
    }
}
